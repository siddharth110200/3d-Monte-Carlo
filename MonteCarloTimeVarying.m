global hbar integfochi Tchig sample1;
rng('shuffle');
%FIXED PARAMETERS
hbar=137.06;
...
%3 mentions of elec/posit to interchange
% storeindex=[];
% avgenergy=[];
% energ={};
...
out=a0*vz*(w2(t)*sin(w2(t)*(t+x))-w1(t)*sin(w1(t)*(t-x)))/2;
end
function out=yforce(~,~,~,~,~,~,~,~,~,~)
out=0;
end
function out=zforce(a0,t,x,~,~,vx,~,~)
out=-a0*((1+vx)*w2(t)*sin(w2(t)*(t+x))+(1-vx)*w1(t)*sin(w1(t)*(t-x)))/2;
end
function out=chig(a0,t,x,~,~,vx,~,vz,freq) %Photon velocity and freq!
global hbar;
out=(hbar^2)*freq*a0*abs(sqrt(((1+vx)*w2(t)*sin(w2(t)*(t+x)))^2+((1-vx)*w1(t)*sin(w1(t)*(t-x)))^2+2*w1(t)*w2(t)*(1-vx^2-2*vz^2)*sin(w1(t)*(t-x))*sin(w2(t)*(t+x))))/2;
end

function out=eta(a0,t,x,~,~,vx,vy,vz)
global hbar;
out=hbar*a0*sqrt(((1+vx)*w2(t)*sin(w2(t)*(t+x)))^2+((1-vx)*w1(t)*sin(w1(t)*(t-x)))^2+2*w1(t)*w2(t)*(1-vx^2-2*vz^2)*sin(w1(t)*(t-x))*sin(w2(t)*(t+x)))/(2*sqrt(1-vx^2-vy^2-vz^2));
end
