...
timeperiod=2*pi/sqrt(w1(0)*w2(0));

%INPUT DATA
a=669;
lambda=0.2;%Global since code obtained through modification
tfinal=100*timeperiod;%Total time for integration
temperature=0.1;
automatic=false;%User input, or randomly selected particles
...
if(~automatic)
    ...
    elec(1)= 4.965/(1.5*10^(-8));
    elec(2)=0;
    elec(3)=0;
    elec(4)=0;%Setting initial values, starting with 1 electron, no positron/photon
    elec(5)=0.4*669;
    elec(6)=-0.07*669;
    elec(7)=0;
    ...
    for k=1:Nelec
        ...
        if(tinelec(k)<tfinal)
            if(elecsol{k}.x(end)<tfinal)
                ...
                chif=fzero(@(lim)radinter(lim,tempet/2)-temprand-eps,[0,tempet/2]);
...
%                     temprand=(1-fine)*rand+fine;
%                     chimf=fzero(@(lim)breitinter(lim,tempchig)-temprand/2-eps,[0,tempchig]);
...
alsoplot1=cellfun(@(solution) atan(sqrt(solution.y(5,end)^2+solution.y(6,end)^2)/solution.y(4,end)), elecsol);
alsoplot2=cellfun(@(solution) atan(sqrt(solution.y(5,end)^2+solution.y(6,end)^2)/solution.y(4,end)), positsol);
alsoplot3=cellfun(@(solution) atan(sqrt(solution.y(5,end)^2+solution.y(6,end)^2)/solution.y(4,end)), photsol);
disp("Electron mean: "+string(mean(alsoplot1))+" Electron SD: "+string(std(alsoplot1))+" Positron mean: "+string(mean(alsoplot2))+" Positron SD: "+string(std(alsoplot2))+" Photon mean: "+string(mean(alsoplot3))+" Photon SD: "+string(std(alsoplot3)));
...
%     end
%     avgenergy=[avgenergy,mean(cellfun(@(solution) sqrt(1+deval(solution,time,4)^2+deval(solution,time,5)^2+deval(solution,time,6)^2), energ))];
% end

%FUNCTIONS
function out=w1(t) %FREQUENCY vs TIME, 1st wave
out=1.5e-8*(1 + 0.002*t*1.5*(10^-8));
end
...
function out=xforce(a0,t,x,~,~,~,vy,vz)
global lambda;
out=(-1/2).*a0.*((vz.*cos((t+(-1).*x).*w1(t))+(-1).*vy.*sin((t+(-1).*x).*w1(t))).*w1(t)+lambda.*((-1).*vz.*cos((t+x).*w2(t))+vy.*sin((t+x).*w2(t))).*w2(t));
end
function out=yforce(a0,t,x,~,~,vx,~,~)
global lambda;
out=(1/2).*a0.*((-1).*((-1)+vx).*sin ((t+(-1).*x).*w1(t)).*w1(t)+lambda.*(1+vx).*sin ((t+x).*w2(t)).*w2(t));
end
function out=zforce(a0,t,x,~,~,vx,~,~)
global lambda;
out=(1/2).*a0.*(((-1)+vx).*cos ((t+(-1).*x).*w1(t)).*w1(t)+(-1).*lambda.*(1+vx).*cos ((t+x).*w2(t)).*w2(t));
end
function out=chig(a0,t,x,~,~,vx,vy,vz,freq) %Photon velocity and freq!
global hbar lambda;
out=(hbar^2)*freq*abs(sqrt((1/4).*a0.^2.*((((-1)+vx).*cos((t+(-1).*x).*w1(t)).*w1(t)+(-1).*lambda.*(1+vx).*cos((t+x).*w2(t)).*w2(t)).^2+(((-1)+vx).*sin((t+(-1).*x).*w1(t)).*w1(t)+(-1).*lambda.*(1+vx).*sin((t+x).*w2(t)).*w2(t)).^2+(-1).*((vz.*cos((t+(-1).*x).*w1(t))+(-1).*vy.*sin((t+(-1).*x).*w1(t))).*w1(t)+lambda.*(vz.*cos((t+x).*w2(t))+(-1).*vy.*sin((t+x).*w2(t))).*w2(t)).^2+((vz.*cos((t+(-1).*x).*w1(t))+(-1).*vy.*sin((t+(-1).*x).*w1(t))).*w1(t)+lambda.*((-1).*vz.*cos((t+x).*w2(t))+vy.*sin((t+x).*w2(t))).*w2(t)).^2)));
end

function out=eta(a0,t,x,~,~,vx,vy,vz)
global hbar lambda;
out=hbar*sqrt((1/4).*a0.^2.*((((-1)+vx).*cos((t+(-1).*x).*w1(t)).*w1(t)+(-1).*lambda.*(1+vx).*cos((t+x).*w2(t)).*w2(t)).^2+(((-1)+vx).*sin((t+(-1).*x).*w1(t)).*w1(t)+(-1).*lambda.*(1+vx).*sin((t+x).*w2(t)).*w2(t)).^2+(-1).*((vz.*cos((t+(-1).*x).*w1(t))+(-1).*vy.*sin((t+(-1).*x).*w1(t))).*w1(t)+lambda.*(vz.*cos((t+x).*w2(t))+(-1).*vy.*sin((t+x).*w2(t))).*w2(t)).^2+((vz.*cos((t+(-1).*x).*w1(t))+(-1).*vy.*sin((t+(-1).*x).*w1(t))).*w1(t)+lambda.*((-1).*vz.*cos((t+x).*w2(t))+vy.*sin((t+x).*w2(t))).*w2(t)).^2))/sqrt(1-vx^2-vy^2-vz^2);
end
